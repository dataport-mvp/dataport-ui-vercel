...
/* ── Education normalizer
   Handles BOTH old field names (classX/intermediate/ug/pg)
   AND new field names from education.js (xSchool/xBoard/xiiSchool etc.)
─────────────────────────────────────────────────────────────────────── */
const normalizeEducation = (ed = {}) => {
  // New format from education.js — flat keys like xSchool, xBoard, degCollege etc.
  ...
  if (isNewFormat) {
    return {
      classX: {
        school:        ed.xSchool,
        board:         ed.xBoard,
        yearOfPassing: ed.xYear,
        resultValue:   ed.xPercent,
      },
      intermediate: {
        school:        ed.xiiSchool,
        board:         ed.xiiBoard,
        yearOfPassing: ed.xiiYear,
        resultValue:   ed.xiiPercent,
      },
      undergraduate: {
        college:       ed.degCollege,
        course:        ed.degName,
        branch:        ed.degBranch,
        yearOfPassing: ed.degYear,
        resultValue:   ed.degPercent,
      },
      postgraduate: {
        college:       ed.pgCollege,
        course:        ed.pgName,
        branch:        ed.pgBranch,
        yearOfPassing: ed.pgYear,
        resultValue:   ed.pgPercent,
      },
    };
  }
...
const normalizeProfile = (snap = {}) => {
  ...
  return {
    ...
    pfRecords:    Array.isArray(snap?.pfRecords)   ? snap.pfRecords
                : Array.isArray(snap?.pf_records)  ? snap.pf_records
                : [],
  };
};
...
export default function EmployerDashboard() {
  ...
  const normalizeConsent = (c) => ({
    ...
    // Message from employer — API stores it as 'message' in DynamoDB
  ...
  const selectConsent = useCallback(async (consent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // profile_snapshot is the employee object (personal + education + uan)
        // employment_snapshot is the employment history array
        setProfileData({
          ...raw,
          profile_snapshot:    normalizeProfile(raw?.profile_snapshot || raw?.employee || {}),
    ...
    <div style={s.page}>
      ...
      <aside style={s.sidebar}>
        ...
        <div style={s.requestBox}>
          ...
          />
          <textarea
            style={{ ...s.sideInput, resize: "vertical", minHeight: 60, lineHeight: 1.4 }}
        ...
        <div style={s.tabRow}>
          {[["pending","Pending"],["completed","Approved"],["rejected","Declined"]].map(([key, label]) => (
            <button
              key={key}
              style={{ ...s.tabBtn, ...(consentTab === key ? s.tabBtnActive : {}) }}
        ...
        <div style={s.consentList}>
          {loading ? (
            ...
            currentList.map(c => {
              ...
              const ts = c.status === "approved" ? (c.approved_at || c.responded_at || c.updated_at)
                       : c.status === "declined" ? (c.responded_at || c.updated_at)
                       : (c.requested_at || c.created_at);
              return (
                <div
                  key={getConsentId(c)}
        ...
        {!selected ? (
          ...
          <div style={s.profileCard}>
            ...
            <div style={s.profileHeader}>
              <div>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 6, flexWrap: "wrap", alignItems: "center" }}>
                  <span style={{
                    ...s.pill,
                    background: selected.status === "approved" ? "#dcfce7" : selected.status === "pending" ? "#fef9c3" : "#fee2e2",
                    color:      selected.status === "approved" ? "#15803d" : selected.status === "pending" ? "#a16207" : "#dc2626",
                  }}>
                ...
                {/* Employer's message — shown prominently */}
            ...
            {selected.status === "pending" && (
              <div style={s.statusMsg}>⏳ Waiting for employee to approve your request.</div>
            )}
            {selected.status === "declined" && (
              <div style={{ ...s.statusMsg, background: "#fef2f2", color: "#dc2626", border: "1px solid #fecaca" }}>❌ Employee declined this request.</div>
            )}
            {selected.status === "approved" && (
              loadingProfile ? (
                ...
                <>
                  {profileData.snapshot_at && (
                    <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 4 }}>
                      📸 Data snapshot taken at: {toIST(profileData.snapshot_at)}
                    </div>
                  )}
                  <div style={s.dataTabs}>
                    {DATA_TABS.map(tab => (
                      <button
                        key={tab}
                        style={{ ...s.dataTab, ...(activeDataTab === tab ? s.dataTabActive : {}) }}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <Card title="Name & Identity">
        <Grid fields={[
          ...
          ["Father's Name", data.fatherName || [data.fatherFirst, data.fatherMiddle, data.fatherLast].filter(Boolean).join(" ")],
...
function EducationTab({ data }) {
  ...
  const sections = [
    ["10th / Class X",          data.classX],
    ["12th / Intermediate",     data.intermediate],
    ["Degree / Undergraduate",  data.undergraduate],
    ["Post Graduation",         data.postgraduate],
      ...
      {sections.map(([title, d]) => (
        <Card key={title} title={title}>
          <Grid fields={[
            ...
            ["Percentage / CGPA",  d.resultValue  || d.result_value],
            ["Mode",               d.mode],
            ["Medium",             d.medium],
            ["Backlogs",           d.backlogs],
          ]} />
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      {list.map((job, i) => (
        <Card key={i} title={`${job.designation || "Employee"} @ ${job.companyName || job.company_name || "—"}`}>
          ...
          {job.gap?.hasGap === "Yes" && (
            <div style={{ marginTop: "0.75rem", paddingTop: "0.75rem", borderTop: "1px solid #f1f5f9" }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 }}>Employment Gap</div>
              <Grid fields={[["Reason", job.gap.reason]]} />
            </div>
          )}
        </Card>
      ))}
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "0.75rem" }}>
      {filled.map(([label, value]) => (
        <div key={label}>
          ...
          <div style={{ fontSize: 14, color: "#0f172a", fontWeight: 500, marginTop: 2, wordBreak: "break-word" }}>{value}</div>
        </div>
      ))}
    </div>
  );
