...
<body style="padding:32px;max-width:900px;margin:0 auto">
  ...
  <div style="border-top:1px solid #e2e8f0;padding-top:10px;margin-top:24px;display:flex;justify-content:space-between">
    ...
    <div style="font-size:9px;color:#94a3b8">Self-reported data, self-declared by the employee. A BGV check establishes verified accuracy for the fields it covers.</div>
  </div>
          ...
          {data.articleships.map((a,i)=>(
            <div key={i} style={{padding:"0.6rem 0.75rem",background:"#fff7ed",borderRadius:6,border:"1px solid #fed7aa",marginBottom:"0.35rem"}}>
              <div className="kv-grid">
                ...
                <KV k="Firm"       v={a.firm} />
                <KV k="Status"     v={a.isOngoing==="Ongoing"?"Ongoing / Pursuing":a.isOngoing==="Completed"?"Completed":(a.isOngoing||"—")} />
                <KV k="City"       v={a.city} />
    ...
    <div>
      ...
      <Sec title="EPFO Declarations &amp; Digital Signature">
        <div className="kv-grid">
          ...
          {!(data.epfoNominees||[]).some(n=>["Spouse","Son","Daughter"].includes(n.relation)) && !(data.epfoNominees||[]).some(n=>["Father","Mother"].includes(n.relation)) && (
          ...
          {(data.gratuityNominees||[]).some(n=>["Father","Mother"].includes(n.relation)) && (
...
function BgvTab({ consentData, apiFetch, API: apiUrl }) {
  ...
  const [expandedVendorHistory, setExpandedVendorHistory] = useState({});
          ...
          <div key={a.assignment_id} style={{border:"2px solid #e2e8f0",borderRadius:12,padding:"1rem",marginBottom:"1.25rem"}}>
            <div onClick={()=>setExpandedVendors(prev=>({...prev,[a.assignment_id]:!prev[a.assignment_id]}))}
              ...
              <div>
                ...
                {/* Assigned is set the moment this case was created, never optional;
                    Completed only appears once a report has actually been filed — see
                    bgv_report_key / the completed-lock. Always visible here, not just
                    inside the report card below, so the timeline is the first thing
                    seen without needing to expand further. */}
                {d?.bgv_assigned_at && (
                  <div style={{fontSize:"0.68rem",color:"#94a3b8",marginTop:"0.15rem"}}>
                    Assigned: {toISTDateTime(d.bgv_assigned_at)}
                    {d?.bgv_completed_at && <> · Completed: {toISTDateTime(d.bgv_completed_at)}</>}
                  </div>
                )}
              </div>
            ...
            {/* Past, completed cases with this SAME vendor — populated whenever this
                vendor was reassigned after an earlier case here was already completed
                (see assign_bgv_vendor's completed-vendor branch, and _bgv_assert_editable
                for why the old one is untouched, never merged into the new one). Shown
                below the current case, oldest-looking-first-in-the-list read as most
                recent past run on top, each with its own Assigned/Completed timestamps
                and full check-by-check results, so the fresh run can be compared
                directly against the old one without hunting through separate views. */}
            {d?.vendor_history?.length > 0 && (
              <div style={{marginTop:"1rem",paddingTop:"0.85rem",borderTop:"1px dashed #cbd5e1"}}>
                <div style={{fontWeight:700,fontSize:"0.8rem",color:"#0f172a",marginBottom:"0.6rem"}}>
                  Past Case{d.vendor_history.length>1?"s":""} With This Vendor ({d.vendor_history.length})
                </div>
                {d.vendor_history.map((h,hi) => {
                  const hOpen = !!expandedVendorHistory[h.assignment_id];
                  return (
                    <div key={h.assignment_id||hi} style={{border:"1.5px solid #e2e8f0",borderRadius:10,padding:"0.7rem 0.85rem",marginBottom:"0.6rem",background:"#f8fafc"}}>
                      <div onClick={()=>setExpandedVendorHistory(prev=>({...prev,[h.assignment_id]:!prev[h.assignment_id]}))}
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:"0.5rem"}}>
                          {h.overall_status && (
                            <span style={{padding:"0.18rem 0.6rem",borderRadius:999,background:`${OVERALL[h.overall_status]||"#64748b"}20`,color:OVERALL[h.overall_status]||"#64748b",fontSize:"0.66rem",fontWeight:800,textTransform:"uppercase"}}>
                              {h.overall_status.toUpperCase()}
                            </span>
                          )}
                          <span style={{fontSize:"0.72rem",color:"#4f46e5",fontWeight:800}}>{hOpen?"▲":"▼"}</span>
                        </div>
                      </div>
                      {hOpen && <>
                        {h.summary && <div style={{fontSize:"0.75rem",color:"#475569",marginTop:"0.5rem",lineHeight:1.5}}>{h.summary}</div>}
                        {h.report_key && (
                          <button onClick={()=>viewReport(h.report_key)} style={{marginTop:"0.5rem",padding:"0.32rem 0.75rem",background:"#fff",color:"#0d6e6e",border:"1.5px solid #0d6e6e",borderRadius:7,fontSize:"0.7rem",fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                            View Old Report ↗
                          </button>
                        )}
                        {h.checks?.length > 0 && (
                          <div style={{marginTop:"0.6rem"}}>
                            {h.checks.map((ch,i) => {
                              const st = CHECK_STATUS_COLORS[ch.status] || CHECK_STATUS_COLORS.pending;
                              return (
                                <div key={i} style={{display:"grid",gridTemplateColumns:"2fr 1.2fr 1fr",gap:"0.5rem",padding:"0.4rem 0",borderBottom:"1px solid #e2e8f0",alignItems:"center"}}>
                                  <div style={{fontSize:"0.74rem",fontWeight:600,color:"#0f172a"}}>{ch.label}</div>
                                  <span style={{display:"inline-block",padding:"0.15rem 0.55rem",borderRadius:999,background:st.bg,color:st.color,fontSize:"0.64rem",fontWeight:700}}>{st.label}</span>
                                  <div style={{fontSize:"0.62rem",color:"#94a3b8"}}>{ch.completed_at ? new Date(ch.completed_at).toLocaleDateString("en-IN",{day:"2-digit",month:"short"}) : "—"}</div>
                                </div>
                              );
                            })}
                          </div>
                  ...
                  <>
                    {(() => {
                      ...
                      return filtered.map(c=>{
                        const hasThread = inboxThreads.some(t=>t.consent_id===c.consent_id);
                        return (
                  ...
                  <div className="pane">
                    ...
                    {selected.status==="approved"&&(
                      ...
                      :<>
                        ...
                        <div className="note-bar">⚠️ <strong>Self-reported data.</strong> All information was filled and submitted by the employee, and remains self-declared regardless of BGV outcome. A BGV check establishes verified accuracy for the fields it covers.</div>
