    ...
    <div style={styles.page}>
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.modalBox}>
            ...
            <div style={{ display: "flex", gap: "1rem", justifyContent: "center" }}>
              <button onClick={() => setShowSignoutConfirm(false)} style={styles.cancelBtn}>Cancel</button>
              <button onClick={() => { logout(); router.push("/employer/login"); }} style={styles.confirmBtn}>Yes, Sign Out</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={styles.card}>
        ...
        <div style={styles.headerRow}>
          ...
          <button style={styles.logoutBtn} onClick={() => setShowSignoutConfirm(true)}>Sign Out</button>
        </div>
        ...
        <div style={styles.searchBox}>
          ...
          <p style={styles.hint}>
            Enter the employee email. A consent request will be sent. Once approved, their profile appears here.
          </p>
        ...
        {/* Consent pending/declined banner */}
        {consentStatus && !employeeData && (
          <div style={styles.statusCard}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <p style={{ margin: 0, fontWeight: "600" }}>Request sent to: <em>{emailInput}</em></p>
                <p style={{ margin: "0.4rem 0 0", fontSize: "0.85rem", color: "#64748b" }}>
                  {consentStatus === "PENDING"  && "Waiting for employee to respond..."}
                  {consentStatus === "DECLINED" && "Employee declined the request."}
                  {consentStatus === "APPROVED" && "Approved — loading data..."}
                </p>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                {statusBadge(consentStatus)}
                {consentStatus === "PENDING" && polling && (
          ...
          <div style={styles.dataSection}>
            <div style={styles.dataHeader}>
              <div>
                ...
                <p style={{ margin: "0.25rem 0 0", fontSize: "0.85rem", color: "#64748b" }}>{employeeData.email || ""}</p>
              </div>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "0.25rem" }}>
                {statusBadge("APPROVED")}
                {employeeData._snapshot_at && (
                  <span style={{ fontSize: "0.75rem", color: "#64748b" }}>
                    📸 Snapshot: {new Date(employeeData._snapshot_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}
                  </span>
                )}
              </div>
            </div>
            ...
            <div style={styles.tabContent}>
              ...
              {activeTab === "Employment" && <EmploymentTab empId={employeeId} token={token} onExpired={handleSessionExpired} />}
        ...
        {myConsents.length > 0 && (
          <div style={{ marginTop: "2.5rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              {[...myConsents].sort((a, b) => b.requested_at - a.requested_at).map(c => (
    ...
    <div>
      {history.snapshot_at && (
        <p style={{ fontSize: "0.78rem", color: "#94a3b8", marginBottom: "1rem" }}>
          📸 Snapshot: {new Date(history.snapshot_at).toLocaleString()}
        </p>
      )}
...
const styles = {
  ...
  dataHeader:  { display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1.25rem" },
  tabNav:      { display: "flex", gap: "0.5rem", marginBottom: "1.5rem", borderBottom: "2px solid #f1f5f9", paddingBottom: "0.5rem", flexWrap: "wrap" },
  ...
  viewBtn:     { padding: "0.4rem 1rem", background: "#2563eb", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer", fontSize: "0.85rem", fontWeight: 600 },
