...
export default function EmployerLogin() {
  ...
  const handle = async () => {
    ...
    try {
      ...
      login(access_token, refresh_token, { role, name: uName || companyName || email, email: uEmail || email });
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .auth-left {
          background: linear-gradient(160deg, #130f2e 0%, #050d1a 60%);
        ...
        .input-field:focus { border-color: rgba(167,139,250,0.4); background: rgba(124,58,237,0.04); }
        ...
        .submit-btn:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 6px 24px rgba(124,58,237,0.5); }
        .submit-btn:disabled { opacity: 0.6; cursor: not-allowed; }

        .forgot-link { text-align: center; }
        .forgot-link a { font-size: 0.82rem; color: #a78bfa; text-decoration: none; }
        .forgot-link a:hover { text-decoration: underline; }

        .divider { display: flex; align-items: center; gap: 1rem; }
        .divider-line { flex: 1; height: 1px; background: rgba(255,255,255,0.06); }
        .divider-text { font-size: 0.75rem; color: #1e1040; }
      ...
      <div className="auth-page">
        ...
        <div className="auth-left">
          ...
          <div className="left-content">
            <div className="left-tag">Employer Portal</div>
            <h2 className="left-title">Hire with<br /><span className="accent">verified confidence.</span></h2>
            <p className="left-desc">Access pre-verified employee records instantly. Eliminate manual background checks and speed up onboarding.</p>
            <div className="left-features">
              <div className="feat-row"><div className="feat-check">✓</div>Request employee consent in seconds</div>
              <div className="feat-row"><div className="feat-check">✓</div>View verified documents — Aadhaar, PAN, education</div>
              <div className="feat-row"><div className="feat-check">✓</div>Employment history with EPFO verification</div>
        ...
        <div className="auth-right">
          <div className="auth-card">
            <div>
              ...
              <div className="auth-sub">{isSignup ? "Start hiring with verified data" : "Sign in to your employer account"}</div>
            </div>

            {isSignup && (
              <div className="input-group">
                <label className="input-label">Company Name</label>
                <input className="input-field" placeholder="Acme Technologies Pvt Ltd" value={companyName} onChange={e => setCompany(e.target.value)} />
              </div>
            )}
