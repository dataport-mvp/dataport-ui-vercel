...
export default function BgvDashboard() {
  ...
  const updateCaseStatus = async (newStatus) => {
    if (!selectedId) return;
    try {
      const res = await apiFetch(`${API}/bgv/status/update`, {
        method: "POST",
        body: JSON.stringify({ consent_id: selectedId, status: newStatus }),
      });
      if (res.ok) {
        setCaseDetail(prev => prev ? {...prev, bgv_status: newStatus} : prev);
        setSaveStatus("✓ Status updated");
        // Refresh cases list so "My Cases" table stays in sync too, not just this view.
        const cRes = await apiFetch(`${API}/bgv/cases`);
        if (cRes.ok) setCases(await cRes.json());
              ...
              <div style={{padding:"1.6rem 1.75rem"}}>
                {[["Current password","password",pwCurrent,setPwCurrent],["New password","password",pwNew,setPwNew],["Confirm new password","password",pwConfirm,setPwConfirm]].map(([label,type,val,setter])=>(
                  <div key={label} style={{marginBottom:"1.1rem"}}>
                    ...
                    <PasswordInput value={val} onChange={e=>setter(e.target.value)} maxLength={label==="Current password"?undefined:128} showCounter={label!=="Current password"} placeholder={label==="Current password"?"":"Enter new password"} inputStyle={{width:"100%",padding:"0.75rem 0.9rem",border:"1.5px solid #e2e8f0",borderRadius:9,fontFamily:"inherit",fontSize:"0.92rem",background:"#f8fafc"}}/>
                    {label!=="Current password" && <div style={{fontSize:"0.72rem",color:"#94a3b8",marginTop:"0.35rem"}}>10+ characters, with a letter, number &amp; symbol</div>}
                  </div>
                ))}
                      ...
                      <div className="case-detail-header">
                        <div>
                          ...
                          <div className="case-subtitle">
                            Assigned by {caseDetail.requestor_name || caseDetail.requestor_email} &nbsp;·&nbsp; {isoDate(caseDetail.bgv_assigned_at)}
                          </div>
                        </div>
                      ...
                      <div className="case-body">
                        ...
                        {(() => {
                          ...
                          const eduLevel = (key,label,icon) => {
                            ...
                            const isUgPg = key === "undergraduate" || key === "postgraduate";
                            const prefix = key === "undergraduate" ? "ug" : key === "postgraduate" ? "pg" : key.toLowerCase();
                            const outsideIndia = e.country === "Outside India";
                            return (
                              <Sec key={key} icon={icon} title={label}>
                                ...
                                {/* Individually named attachments, not a generic "view all" button —
                                    each shown only when it actually applies to this level/country. */}
                                {isUgPg ? (<>
                                  {docLink("education",`${prefix}_provisional`) && <div style={{marginTop:"0.7rem"}}>Provisional Marksheet {docLink("education",`${prefix}_provisional`)}</div>}
                                  {docLink("education",`${prefix}_convocation`) && <div style={{marginTop:"0.5rem"}}>Convocation Certificate {docLink("education",`${prefix}_convocation`)}</div>}
                                  {outsideIndia && docLink("education",`${prefix}_equivalency`) && <div style={{marginTop:"0.5rem"}}>Equivalency Certificate {docLink("education",`${prefix}_equivalency`)}</div>}
                                </>) : (<>
                                  {docLink("education",key) && <div style={{marginTop:"0.7rem"}}>Upload {label.split(" —")[0]} Certificate {docLink("education",key)}</div>}
                                  {outsideIndia && docLink("education",`${prefix}_equivalency`) && <div style={{marginTop:"0.5rem"}}>Equivalency Certificate {docLink("education",`${prefix}_equivalency`)}</div>}
                                </>)}
                              </Sec>
                            );
                  ...
                  <>
                    ...
                    {(inboxSearch
                      ...
                      <div key={t.consent_id} className={`inbox-thread${activeThread===t.consent_id?" active":""}`} onClick={()=>loadThread(t.consent_id)}>
                        ...
                        <div style={{fontSize:"0.72rem",color:"#64748b",margin:"0.15rem 0"}}>{t.other_party_name}</div>
