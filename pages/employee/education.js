...
export default function EducationDetails() {
  ...
  const [xYear, setXYear] = useState(""); const [xResultType, setXResultType] = useState(""); const [xResultValue, setXResultValue] = useState(""); const [xMedium, setXMedium] = useState("");
  const [xCertKey, setXCertKey] = useState("");
  ...
  const [iYear, setIYear] = useState(""); const [iResultType, setIResultType] = useState(""); const [iResultValue, setIResultValue] = useState(""); const [iMedium, setIMedium] = useState("");
  const [iCertKey, setICertKey] = useState("");
  ...
  const [ugCertKey, setUgCertKey] = useState("");
  ...
  const [pgCertKey, setPgCertKey] = useState("");
    ...
    const fetchDraft = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const x = edu.classX || {}; const i = edu.intermediate || {}; const ug = edu.undergraduate || {}; const pg = edu.postgraduate || {};
          ...
          if (x.yearOfPassing) setXYear(x.yearOfPassing); if (x.resultType) setXResultType(x.resultType); if (x.resultValue) setXResultValue(x.resultValue); if (x.medium) setXMedium(x.medium);
          if (x.certKey) setXCertKey(x.certKey);
          ...
          if (i.yearOfPassing) setIYear(i.yearOfPassing); if (i.resultType) setIResultType(i.resultType); if (i.resultValue) setIResultValue(i.resultValue); if (i.medium) setIMedium(i.medium);
          if (i.certKey) setICertKey(i.certKey);
          ...
          if (ug.certKey) setUgCertKey(ug.certKey);
          ...
          if (pg.certKey) setPgCertKey(pg.certKey);
        }
      } catch (_) {}
      setLoading(false);
    };
  ...
  const buildEducation = () => ({
    ...
    postgraduate:  { college: pgCollege, university: pgUniversity, course: pgCourse, hallTicket: pgHall, from: pgFrom, to: pgTo, address: pgAddress, mode: pgMode, yearOfPassing: pgYear, resultType: pgResultType, resultValue: pgResultValue, backlogs: pgBacklogs, medium: pgMedium, certKey: pgCertKey },
  });
  ...
  const saveDraft = async () => {
    ...
    const res = await apiFetch(`${API}/employee`, {
      method: "POST",
      body: JSON.stringify({
        employee_id: d.employee_id, status: d.status || "draft",
        firstName: d.firstName || "", lastName: d.lastName || "", middleName: d.middleName,
        fatherName: d.fatherName, fatherFirst: d.fatherFirst, fatherMiddle: d.fatherMiddle, fatherLast: d.fatherLast,
        dob: d.dob, gender: d.gender, nationality: d.nationality, mobile: d.mobile || "", email: d.email,
        aadhaar: d.aadhaar, pan: d.pan, passport: d.passport, aadhaarKey: d.aadhaarKey, panKey: d.panKey,
        currentAddress: d.currentAddress, permanentAddress: d.permanentAddress,
        uanNumber: d.uanNumber, nameAsPerUan: d.nameAsPerUan, mobileLinked: d.mobileLinked, isActive: d.isActive, pfRecords: d.pfRecords,
  ...
  const handlePrevious = async () => { try { await saveDraft(); } catch (_) {} router.push("/employee/personal"); };
  const handleSignout  = async () => { try { await saveDraft(); } catch (_) {} logout(); };
    ...
    <div style={styles.page}>
      ...
      <div style={styles.card}>
        ...
        <Section title="Class X">
          <Row><Input label="School Name" value={xSchool} onChange={setXSchool} /><Input label="Board Name" value={xBoard} onChange={setXBoard} /><Input label="Hall Ticket / Roll Number" value={xHall} onChange={setXHall} /></Row>
          <Row><Input type="date" label="From" value={xFrom} onChange={setXFrom} /><Input type="date" label="To" value={xTo} onChange={setXTo} /></Row>
          <Input label="School Address" value={xAddress} onChange={setXAddress} />
          <Row><Input label="Year of Passing" value={xYear} onChange={setXYear} /><Select label="Result Type" value={xResultType} onChange={setXResultType} options={["Percentage","Grade"]} /><Input label="Result Value" value={xResultValue} onChange={setXResultValue} /></Row>
          <Input label="Medium of Study" value={xMedium} onChange={setXMedium} />
          <FileUpload label="Upload Class X Certificate" category="education" subKey="classX" apiFetch={apiFetch} value={xCertKey} onChange={setXCertKey} />
        </Section>

        <Section title="Intermediate">
          <Row><Input label="College Name" value={iCollege} onChange={setICollege} /><Input label="Board Name" value={iBoard} onChange={setIBoard} /><Input label="Hall Ticket / Roll Number" value={iHall} onChange={setIHall} /></Row>
          <Row><Input type="date" label="From" value={iFrom} onChange={setIFrom} /><Input type="date" label="To" value={iTo} onChange={setITo} /></Row>
          <Input label="College Address" value={iAddress} onChange={setIAddress} />
          <Row><Select label="Mode of Education" value={iMode} onChange={setIMode} options={["Full-time","Part-time","Distance"]} /><Input label="Year of Passing" value={iYear} onChange={setIYear} /></Row>
          <Row><Select label="Result Type" value={iResultType} onChange={setIResultType} options={["Percentage","Grade"]} /><Input label="Result Value" value={iResultValue} onChange={setIResultValue} /></Row>
          <Input label="Medium of Study" value={iMedium} onChange={setIMedium} />
          <FileUpload label="Upload Intermediate Certificate" category="education" subKey="intermediate" apiFetch={apiFetch} value={iCertKey} onChange={setICertKey} />
        </Section>

        <Section title="Undergraduate (UG)">
          <Row><Input label="College Name" value={ugCollege} onChange={setUgCollege} /><Input label="University Name" value={ugUniversity} onChange={setUgUniversity} /><Input label="Course / Degree" value={ugCourse} onChange={setUgCourse} /></Row>
          <Row><Input label="Hall Ticket / Roll Number" value={ugHall} onChange={setUgHall} /><Select label="Mode of Education" value={ugMode} onChange={setUgMode} options={["Full-time","Part-time","Distance"]} /></Row>
          <Row><Input type="date" label="From" value={ugFrom} onChange={setUgFrom} /><Input type="date" label="To" value={ugTo} onChange={setUgTo} /></Row>
          <Input label="College Address" value={ugAddress} onChange={setUgAddress} />
          <Row><Input label="Year of Passing" value={ugYear} onChange={setUgYear} /><Select label="Result Type" value={ugResultType} onChange={setUgResultType} options={["Percentage","CGPA","Grade"]} /><Input label="Result Value" value={ugResultValue} onChange={setUgResultValue} /></Row>
          ...
          <FileUpload label="Upload UG Degree / Provisional Certificate" category="education" subKey="undergraduate" apiFetch={apiFetch} value={ugCertKey} onChange={setUgCertKey} />
        </Section>

        <Section title="Postgraduate (PG)">
          <Row><Input label="College Name" value={pgCollege} onChange={setPgCollege} /><Input label="University Name" value={pgUniversity} onChange={setPgUniversity} /><Input label="Course / Degree" value={pgCourse} onChange={setPgCourse} /></Row>
          <Row><Input label="Hall Ticket / Roll Number" value={pgHall} onChange={setPgHall} /><Select label="Mode of Education" value={pgMode} onChange={setPgMode} options={["Full-time","Part-time","Distance"]} /></Row>
          <Row><Input type="date" label="From" value={pgFrom} onChange={setPgFrom} /><Input type="date" label="To" value={pgTo} onChange={setPgTo} /></Row>
          <Input label="College Address" value={pgAddress} onChange={setPgAddress} />
          <Row><Input label="Year of Passing" value={pgYear} onChange={setPgYear} /><Select label="Result Type" value={pgResultType} onChange={setPgResultType} options={["Percentage","CGPA","Grade"]} /><Input label="Result Value" value={pgResultValue} onChange={setPgResultValue} /></Row>
          ...
          <FileUpload label="Upload PG Degree / Provisional Certificate" category="education" subKey="postgraduate" apiFetch={apiFetch} value={pgCertKey} onChange={setPgCertKey} />
        </Section>
