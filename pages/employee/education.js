...
const STEP_DONE_BG = "#fffbeb";
const STEP_DONE_CK = "#d97706";
const STEP_CONN    = "#fde68a";
const STEP_SHADOW  = "rgba(217,119,6,0.28)";
...
const G = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Split attachment boxes */
  .att-split { display: flex; gap: 0.9rem; flex-wrap: wrap; margin-top: 0.85rem; }
  .att-box { flex: 1; min-width: 200px; background: #f8f9fc; border: 1.5px solid #e4e2ed;
    border-radius: 10px; padding: 0.9rem 1rem; }
  .att-box-lbl { font-size: 0.7rem; font-weight: 700; color: #64748b; letter-spacing: 0.55px;
...
// ─── StepNav — P2 active accent: AMBER ───────────────────────────────
    ...
    const fetchDraft = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (ug.provKey) setUgProvKey(ug.provKey); if (ug.convoKey) setUgConvoKey(ug.convoKey);
          // backwards compat: if old certKey used, map to provKey
          if (!ug.provKey && ug.certKey) setUgProvKey(ug.certKey);
          ...
          if (pg.provKey) setPgProvKey(pg.provKey); if (pg.convoKey) setPgConvoKey(pg.convoKey);
          if (!pg.provKey && pg.certKey) setPgProvKey(pg.certKey);
          ...
          <div className="sc amb">
            ...
            <div className="att-split">
              <div className="att-box">
                <AL lbl="Provisional Marksheet"/>
                <FileUpload label="Upload Provisional Marksheet" category="education" subKey="ug_provisional" apiFetch={apiFetch} value={ugProvKey} onChange={(k)=>{setUgProvKey(k);isDirtyRef.current=true;}}/>
              </div>
              <div className="att-box">
                <span className="att-box-lbl">Convocation Certificate</span>
                <FileUpload label="Upload Convocation Certificate" category="education" subKey="ug_convocation" apiFetch={apiFetch} value={ugConvoKey} onChange={(k)=>{setUgConvoKey(k);isDirtyRef.current=true;}}/>
              </div>
            </div>
          </div>

          {/* PG toggle — split attachments */}
          <div className="sc vio">
            ...
            {hasPG==="Yes" && (<>
              ...
              <div className="att-split">
                <div className="att-box">
                  <AL lbl="Provisional Marksheet"/>
                  <FileUpload label="Upload Provisional Marksheet" category="education" subKey="pg_provisional" apiFetch={apiFetch} value={pgProvKey} onChange={(k)=>{setPgProvKey(k);isDirtyRef.current=true;}}/>
                </div>
                <div className="att-box">
                  <span className="att-box-lbl">Convocation Certificate</span>
                  <FileUpload label="Upload Convocation Certificate" category="education" subKey="pg_convocation" apiFetch={apiFetch} value={pgConvoKey} onChange={(k)=>{setPgConvoKey(k);isDirtyRef.current=true;}}/>
                </div>
              </div>
            </>)}
          </div>
