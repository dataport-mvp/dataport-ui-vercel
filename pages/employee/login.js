...
function Logo({ accentColor = "#16a34a" }) {
  const css = `
...
function HandshakeIllustration({ accent = "#16a34a" }) {
  return (
    <svg width="100%" viewBox="0 0 300 200" fill="none" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <style>{`
          ...
          @keyframes gl{0%,100%{opacity:.15}50%{opacity:.3}}
      ...
      <g className="hsg">
        ...
        <circle cx="243" cy="65" r="2" fill="#2563eb"/>
        <circle cx="253" cy="65" r="2" fill="#2563eb"/>
        <path d="M243 74 Q248 78 253 74" fill="none" stroke="#2563eb" strokeWidth="1.5" strokeLinecap="round"/>
        ...
        <ellipse cx="150" cy="120" rx="7" ry="3.5" fill="#4ade80" opacity="0.3"/>

        {/* Floating consent dots */}
        <circle cx="98"  cy="96" r="3.5" fill={accent} opacity="0.7"/>
        <circle cx="114" cy="88" r="2.5" fill={accent} opacity="0.5"/>
        <circle cx="202" cy="96" r="3.5" fill="#2563eb" opacity="0.6"/>
        <circle cx="186" cy="88" r="2.5" fill="#2563eb" opacity="0.4"/>
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .lft::before{content:'';position:absolute;width:500px;height:500px;border-radius:50%;background:radial-gradient(circle,rgba(22,163,74,0.07) 0%,transparent 70%);top:-80px;left:-80px;pointer-events:none}
        .lft-body{position:relative;z-index:1;padding-top:2.5rem}
        .tag{font-size:.67rem;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:#16a34a;margin-bottom:1.1rem;display:flex;align-items:center;gap:.5rem}
        .tag-line{width:18px;height:1.5px;background:#16a34a}
        ...
        .ttl span{color:#16a34a}
        .dsc{font-size:.86rem;color:#475569;line-height:1.75;margin-bottom:2rem;max-width:330px}
        .stps{display:flex;flex-direction:column;gap:.85rem;margin-bottom:2rem}
        .stp{display:flex;align-items:flex-start;gap:.8rem}
        .stn{width:24px;height:24px;border-radius:6px;background:rgba(22,163,74,0.1);border:1px solid rgba(22,163,74,0.25);display:flex;align-items:center;justify-content:center;font-size:.68rem;font-weight:700;color:#16a34a;flex-shrink:0}
        .stb-t{font-size:.8rem;font-weight:600;color:#94a3b8;margin-bottom:2px}
        .stb-d{font-size:.76rem;color:#334155;line-height:1.5}
        ...
        .fin:focus{border-color:rgba(22,163,74,0.45);background:rgba(22,163,74,0.04)}
        ...
        .pht{font-size:.68rem;color:#1e3a5f}
        ...
        .sub{padding:.8rem;background:#16a34a;color:#fff;border:none;border-radius:8px;font-family:'DM Sans',sans-serif;font-size:.88rem;font-weight:700;cursor:pointer;transition:background .15s}
        .sub:hover:not(:disabled){background:#15803d}
        .sub:disabled{opacity:.5;cursor:not-allowed}
        .fgt{text-align:center}
        .fgt a{font-size:.78rem;color:#16a34a;text-decoration:none}
        ...
        .dvt{font-size:.7rem;color:#1e3a5f}
        ...
        .cft a{font-size:.68rem;color:#1e3a5f;text-decoration:none;transition:color .15s}
      ...
      <div className="pg">
        <div className="lft">
          <Link href="/"><Logo accentColor="#16a34a"/></Link>
          <div className="lft-body">
            <div className="tag"><span className="tag-line"/>Employee Portal</div>
            <h2 className="ttl">Your career data,<br/><span>your control.</span></h2>
            <p className="dsc">Build your verified employment profile once. Share it securely with any employer — with your consent, every single time.</p>
            <div className="stps">
              {[["1","Create your profile","Personal, education, employment and documents — filled once, forever."],["2","Upload documents","Aadhaar, PAN, certificates — encrypted and stored securely."],["3","Approve and share","Every employer request needs your approval. You decide who sees what."]].map(([n,t,d])=>(
            ...
            <HandshakeIllustration accent="#16a34a"/>
          </div>
          <div className="lft-ft">
            <span className="fc">© 2026 Datagate</span>
            <div className="fl"><a href="/privacy" target="_blank" rel="noopener noreferrer" className="fla">Privacy Policy</a></div>
          </div>
        </div>
        ...
        <div className="rgt">
          <div className="fcd">
            ...
            {isSignup&&(<>
              ...
              <div className="fld"><label className="flb">Mobile Number</label><input className="fin" placeholder="9876543210" value={phone} onChange={e=>handlePhone(e.target.value)} inputMode="numeric" maxLength={10}/><span className="pht">{phone.length}/10 digits</span></div>
            </>)}
            ...
            <div className="cft"><a href="/privacy" target="_blank" rel="noopener noreferrer">Privacy Policy</a></div>
          </div>
        </div>
      </div>
    </>
