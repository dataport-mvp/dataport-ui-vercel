...
const STEP_DONE_BG = "#f0fdf4";
const STEP_DONE_CK = "#16a34a";
const STEP_CONN    = "#bbf7d0";
const STEP_SHADOW  = "rgba(22,163,74,0.28)";
...
const G = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sc { background: #fff; border-radius: 14px; padding: 1.5rem 1.6rem; margin-bottom: 1.1rem;
  ...
  .ack-item { display: flex; gap: 0.75rem; align-items: flex-start; padding: 0.85rem 1rem;
    ...
    border: 1.5px solid #e4e2ed; background: #f8f9fc; }
  ...
  .ack-check { width: 20px; height: 20px; border-radius: 5px; flex-shrink: 0; margin-top: 1px;
    border: 2px solid #e4e2ed; background: #fff; display: flex; align-items: center;
...
// ─── StepNav — P4 active accent: GREEN ───────────────────────────────
          ...
          if (dr.uanNumber || dr.nameAsPerUan || dr.mobileLinked || dr.isActive || dr.pfRecords) {
            setForm({
              ...
              pfRecords: dr.pfRecords && dr.pfRecords.length > 0
                ? dr.pfRecords.map(r => ({ companyName:r.companyName||"", pfMemberId:r.pfMemberId||"", dojEpfo:r.dojEpfo||"", doeEpfo:r.doeEpfo||"", pfTransferred:r.pfTransferred||"" }))
                : [emptyPfRecord()],
            });
          }
        }
