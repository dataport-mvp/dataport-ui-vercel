...
function FDate({ l, v, s, r=true, errKey, errors, onFix }) {
  ...
  const handleChange=(e)=>{
    ...
    // Reject out-of-range day/month as soon as it's unambiguous, same as the other
    // date fields — this used to only check month/day range once all 10 characters
    // were typed, which meant an invalid date like "19/15/2021" would sit in the box
    // looking accepted (setRaw ran unconditionally) while silently never actually
    // committing a value — confusing either way. Now it's rejected immediately.
    const parts = val.split("/");
        ...
        <div className="wrap">
            ...
            {showUanFields && (
              <>
                ...
                <div style={{marginTop:"0.75rem"}}>
                  ...
                  <p style={{fontSize:"0.7rem",color:"#6b6894",marginBottom:"0.4rem",fontWeight:500,lineHeight:1.5}}>Download from EPFO Member Portal and upload a snapshot of your entire service history.</p>
          ...
          {hasUan === "yes" && (
            <div className="sc grn" style={{marginBottom:"1.1rem"}}>
              ...
              {nominees.length < 4 && nominees.reduce((s,n)=>s+(parseInt(n.share)||0),0) < 100 && <button className="add-btn" onClick={()=>{setNominees(p=>[...p,makeNominee()]);flagPostSignEdit();}}>+ Add Another Nominee</button>}
              {nominees.length < 4 && nominees.reduce((s,n)=>s+(parseInt(n.share)||0),0) >= 100 && nominees.length > 0 && (
