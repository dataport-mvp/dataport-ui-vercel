...
const ACK_ITEMS = [
  { id: "truthful",      text: "I hereby declare that all information provided in this profile is true, accurate and complete to the best of my knowledge and belief." },
  { id: "consent_aware", text: "I understand that my data will only be shared with an employer or BGV agency after I explicitly approve their consent request. I can decline or revoke access at any time." },
  { id: "docs_genuine",  text: "I confirm that all documents, certificates and identity proofs I have referenced are genuine and have not been tampered with." },
  { id: "no_suppress",   text: "I have not wilfully withheld or suppressed any material information regarding my employment history, qualifications or legal record." },
  { id: "liability",     text: "I accept that any false or misleading information may result in immediate disqualification, termination of employment and/or legal action." },
];

const emptyPfRecord = () => ({ companyName: "", pfMemberId: "", dojEpfo: "", doeEpfo: "", pfTransferred: "" });

// KEY FIX: same pattern as personal.js — use ?? "" so empty strings restore correctly
const s = (val) => val ?? "";
...
export default function UANPage() {
  ...
  /* ── Restore draft ───────────────────────────────────────────────────────── */
  useEffect(() => {
    // 1. localStorage first (fast — mid-session)
    try {
      const saved    = localStorage.getItem("dg_uan");
      const savedAck = localStorage.getItem("dg_uan_acks");
      if (saved) {
        setForm(JSON.parse(saved));
        if (savedAck) setCheckedAcks(JSON.parse(savedAck));
        return;
      }
    } catch (_) {}

    // 2. API fallback (re-login — localStorage cleared by logout)
    // KEY FIX: uses ?? "" on every field so "" restores correctly — not just truthy values
    if (!token) return;
    fetch(`${API}/employee/draft`, { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then(d => {
        ...
        // Build restored form — ?? "" means empty string IS restored (unlike old "|| """)
        // Old bug: d.uanNumber || "" — if uanNumber="" it evaluates to "" fine, BUT
        // the old setForm only ran inside "if (d.uanNumber)" which skipped "" entirely.
        const restored = {
          uanMaster: {
            uanNumber:    s(d.uanNumber),
            nameAsPerUan: s(d.nameAsPerUan),
            mobileLinked: s(d.mobileLinked),
            isActive:     s(d.isActive),
          },
          // Filter out ALL-empty pfRecords — don't restore placeholder blank cards
          // A record is real if at least one field has content
          pfRecords: (d.pfRecords || []).filter(r =>
            Object.values(r).some(v => v && String(v).trim() !== "")
          ).length > 0
            ? d.pfRecords.filter(r => Object.values(r).some(v => v && String(v).trim() !== ""))
            : [emptyPfRecord()],
        };
        setForm(restored);
  ...
  const updatePf = (index, field, value) =>
    setForm(prev => ({
      ...prev,
      pfRecords: prev.pfRecords.map((r, i) => i === index ? { ...r, [field]: value } : r),
    }));
  ...
  const validate = () => {
    const e = {};
    if (!form.uanMaster.uanNumber.trim())    e.uanNumber    = "Required";
    if (!form.uanMaster.nameAsPerUan.trim()) e.nameAsPerUan = "Required";
    setErrors(e);
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Step 1: Fetch full existing record — never rely on localStorage for other pages' data
      ...
      // Step 2: Build ack profile
      const ackProfile = {};
      ACK_ITEMS.forEach(a => { ackProfile[a.id] = checkedAcks[a.id] ? "Yes" : "No"; });

      // Step 3: Merge — existing + UAN fields + submitted status
      ...
      // Step 5: POST employment history if present
      ...
      // Step 6: Clear draft caches — keep dg_employee_id for re-login
      ["dg_personal", "dg_education", "dg_employments", "dg_ack", "dg_uan", "dg_uan_acks"]
      ...
      {showSignoutConfirm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#fff", borderRadius: "14px", padding: "2rem", maxWidth: "400px", width: "90%", textAlign: "center", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            <p style={{ color: "#475569", marginBottom: "1.5rem", fontSize: "0.9rem" }}>Your data is saved. You can continue from where you left off after logging back in.</p>
          ...
          <div style={styles.row}>
            <div>
              <Field label="UAN Number" required error={errors.uanNumber}>
                <input
                  style={{ ...styles.input, borderColor: errors.uanNumber ? "#dc2626" : "#cbd5e1" }}
                  maxLength={12}
              ...
              <div style={{ marginTop: "1rem" }}>
                <Field label="Name as per UAN" required error={errors.nameAsPerUan}>
                  <input
                    style={{ ...styles.input, borderColor: errors.nameAsPerUan ? "#dc2626" : "#cbd5e1" }}
            ...
            <div key={index} style={{ marginBottom: "2rem" }}>
              <div style={styles.row}>
                <div>
                  ...
                  <input style={styles.input} value={record.companyName} onChange={e => updatePf(index, "companyName", e.target.value)} />
                </div>
                <div>
                  <label style={styles.label}>PF Member ID</label>
                  <input style={styles.input} value={record.pfMemberId} onChange={e => updatePf(index, "pfMemberId", e.target.value)} />
                </div>
              </div>
              <div style={styles.row}>
                <div>
                  <label style={styles.label}>Date of Joining (EPFO)</label>
                  <input type="date" style={styles.input} value={record.dojEpfo} onChange={e => updatePf(index, "dojEpfo", e.target.value)} />
                </div>
                <div>
                  <label style={styles.label}>Date of Exit (EPFO)</label>
                  <input type="date" style={styles.input} value={record.doeEpfo} onChange={e => updatePf(index, "doeEpfo", e.target.value)} />
                </div>
              </div>
              <div>
                ...
                <div style={styles.pillContainer}>
                  {["Yes", "No"].map(val => (
                    <div key={val} style={styles.pill(record.pfTransferred === val)} onClick={() => updatePf(index, "pfTransferred", val)}>{val}</div>
                  ))}
                </div>
              </div>
              {index > 0 && (
                <button style={styles.removeBtn} onClick={() => setForm({ ...form, pfRecords: form.pfRecords.filter((_, i) => i !== index) })}>
                  − Remove This Company
                </button>
              )}
            </div>
          ))}
          <button style={styles.addBtn} onClick={() => setForm({ ...form, pfRecords: [...form.pfRecords, emptyPfRecord()] })}>
            + Add Another Company
          </button>
        </div>
...
const styles = {
  ...
  ackCheck: (checked) => ({ width: "18px", height: "18px", minWidth: "18px", borderRadius: "4px", marginTop: "2px", border: checked ? "none" : "2px solid #cbd5e1", background: checked ? "#2563eb" : "#fff", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }),
